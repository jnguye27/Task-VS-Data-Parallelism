/* 
Program: A Simple Ray Tracer (Data Parallelism)
...
int output = 0;
   /* The variable that I made global */
   /* Will contain the raw image, dynamically allocate framebuffer */
unsigned char *img;
...
/* Calculates the colors of each pixel in one row */
void *rowColorCalculator(void* rank){
   /* Declaring Variables */
   long my_rank;
   int x, y, local_m, my_first_row, my_last_row;
   ray r;
   ...
   /* Calculate the pixel colors based on row values */
   for(y = my_first_row; y <= my_last_row; y++){
      for(x = 0; x < WIDTH; x++){
         float red = 0;
         float green = 0;
         float blue = 0;
...
int main(int argc, char *argv[]){
   ...
   /* Declaring Variables */
   long int  currentThread, t1, t2, time;
   ...
   /* Calculate the pixel colours by splitting them between the number of threads given */
   for(currentThread = 0; currentThread < threads; currentThread++){
      pthread_create(&thread_handles[currentThread], NULL, rowColorCalculator, (void*) currentThread);
   }
